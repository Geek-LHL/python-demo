...
> - C、本项目基于python 模块 flask 搭建的web项目 （有兴趣可以看看Django的使用）
> - D、如果要连接mysql数据库，请修改settings.config.yml 文件配置
...
## 3、requestsA.py：http请求的常用使用方式（类似于java里的OKHttpUtils）

## 4、restful：提供了对外暴露的接口
 > - A、启动项目，访问接口：localhost:8000/test/t

## 5、cron：定时器相关使用（spring boot的 @Scheduled(cron = "0 */10 * * * ? ") 功能一模一样）
> - A、@Scheduled注解的实现方式，参照app.cron.__init__.py类

## 6、微信机器人：app.wx.wx_test.py 简单的列出了python怎样去发送微信消息

## 7、settings: 提供了全局配置，如果数据库用户秘密。。。

## 8、.env：该文件是控制项目的运行环境（本地：dev  测试：test  正式：product）
> - A、修改.env文件内容：在jenkins发布时，使用命令sed -i 's/ENVSTATUS=dev/ENVSTATUS=${CENV}/g' .env 去修改

## 9、Pipfile：是基于pipen创建的，项目第三方包依赖（类似于nodejs包依赖）
> - A、阿里云源：url = "https://mirrors.aliyun.com/pypi/simple"
> - B、使用python3.6搭建：python_version = "3.6"
...
> - C、IDEA里运行项目: 右键 run.py 运行 （可以debug运行）
> - D、linux下运行项目: nohup pipenv run python manager.py server >> ../run.log &
...
B、linux源码安装python3.6，用于pipenv使用3.6创建虚拟环境
    ...
    15、常见错误：
        A、错误1：
           pkg_resources.DistributionNotFound: The 'cffi>=1.11.5; sys_platform == "win32" and platform_python_implementation == "CPython"'...
